...
class Block(nn.Module):
    ...
    def forward(self, x, get_att=False, prompt=None):
        # if get_att:
        #     if prompt != None:
        #         l = x.shape[1]
        #         x = torch.cat([x, prompt], dim=1)
        #         x1, atten = self.attn(self.norm1(x), get_attn=True)
        #         x = x + self.drop_path(x1)
        #         x_ = x + self.drop_path(self.mlp(self.norm2(x)))
        #         x = x_[:, :l]
        #         prompt = x_[:, l:]
        #         return x, atten, prompt
        #     else:
        #         x1, atten = self.attention(self.norm1(x), get_attn=True)
        #         x = x + self.drop_path(x1)
        #         x_ = x + self.drop_path(self.mlp(self.norm2(x)))
        #         return x_, atten
...
class Trans(nn.Module):
    ...
    def forward(self, x, cam_label=None, view_label=None, get_embedding=False):
        ...
        # self.view_num = 0
        # if self.cam_num > 1 and self.view_num > 1:
        #     x = x + self.pos_embed + self.sie_xishu * self.sie_embed[cam_label * self.view_num + view_label]
        # elif self.cam_num > 1:
        #     x = x + self.pos_embed + self.sie_xishu * self.sie_embed[cam_label]
        # elif self.view_num > 1:
        #     x = x + self.pos_embed + self.sie_xishu * self.sie_embed[view_label]
        # else:
