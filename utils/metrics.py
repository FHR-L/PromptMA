...
class R1_mAP_eval_cross_modal():
    def __init__(self, num_query, max_rank=50, feat_norm=True, reranking=False):
        super(R1_mAP_eval_cross_modal, self).__init__()
        self.num_query = num_query
        self.max_rank = max_rank
        self.feat_norm = feat_norm
        self.reranking = reranking
    ...
    def compute(self):  # called after each epoch
        ...
        # query
        qf = feats1[:self.num_query]
        q_pids = np.asarray(self.pids[:self.num_query])
        q_camids = np.asarray(self.camids[:self.num_query])
        # gallery
        gf = feats2[self.num_query:]
        g_pids = np.asarray(self.pids[self.num_query:])
...
class R1_mAP_cross_modal():
    def __init__(self, num_query, max_rank=50, feat_norm='yes'):
        super(R1_mAP_cross_modal, self).__init__()
        self.num_query = num_query
        self.max_rank = max_rank
        self.feat_norm = feat_norm
    ...
    def update(self, output):
        feat1, feat2, pid, camid, sceneid, img_path = output
